...
class UserController {
  async store(requisicao, resposta) {
    try {
      const novoUser = await _User2.default.create(requisicao.body);
  ...
  async index(requisicao, resposta) {
    try {
      ...
      const users = await _User2.default.findAll({ attributes: ['id', 'nome', 'email'] }); // passamos os atributos que queremos exibir caso a busca retorne true
  ...
  async show(requisicao, resposta) {
    try {
      ...
      const user = await _User2.default.findByPk(requisicao.params.id); // retornará todos os usuários na base de dados.
  ...
  async update(requisicao, resposta) {
    try {
      ...
      const users = await _User2.default.findByPk(requisicao.userId);
...
exports. default = new UserController(); // quando exportamos a classe desssa forma, o objeto já vai estar instanciado ao realizar o import.
